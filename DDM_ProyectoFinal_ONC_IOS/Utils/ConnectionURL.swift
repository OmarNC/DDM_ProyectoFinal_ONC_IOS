...
class ConnectionURL
{
    ...
    static func getURLWeather(lat: Double, lon: Double) -> String {
        return "\(URL_BASE)weather?lang=se&units=metric&lat=\(String(lat))&lon=\(String(lon))&appid=\(OPEN_WEATHER_API_KEY)"
    }
    
    static func getURLWeatherForecast(lat: Double, lon: Double) -> String {
        return "\(URL_BASE)forecast?lang=se&units=metric&lat=\(String(lat))&lon=\(String(lon))&appid=\(OPEN_WEATHER_API_KEY)"
    }
